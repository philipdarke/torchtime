...
class TestPhysioNet2019Binary:
    ...
    def test_train_val(self):
        ...
        # Check data set size
        assert dataset.X_train.shape == torch.Size([283, 72, 41])
        assert dataset.y_train.shape == torch.Size([283, 1])
        assert dataset.length_train.shape == torch.Size([283])
        assert dataset.X_val.shape == torch.Size([120, 72, 41])
        assert dataset.y_val.shape == torch.Size([120, 1])
    ...
    def test_train_val_test(self):
        ...
        assert dataset.y_train.shape == torch.Size([283, 1])
        assert dataset.length_train.shape == torch.Size([283])
        assert dataset.X_val.shape == torch.Size([80, 72, 41])
        assert dataset.y_val.shape == torch.Size([80, 1])
        assert dataset.length_val.shape == torch.Size([80])
        assert dataset.X_test.shape == torch.Size([40, 72, 41])
        assert dataset.y_test.shape == torch.Size([40, 1])
    ...
    def test_no_impute(self):
        ...
        assert torch.sum(torch.isnan(dataset.X_train)).item() == 690400
        assert torch.sum(torch.isnan(dataset.y_train)).item() == 0
        assert torch.sum(torch.isnan(dataset.X_val)).item() == 194691
        assert torch.sum(torch.isnan(dataset.y_val)).item() == 0
        assert torch.sum(torch.isnan(dataset.X_test)).item() == 96444
    ...
    def test_no_time(self):
        ...
        # Check data set size
        assert dataset.X_train.shape == torch.Size([283, 72, 40])
        assert dataset.X_val.shape == torch.Size([80, 72, 40])
        assert dataset.X_test.shape == torch.Size([40, 72, 40])
    ...
    def test_mask(self):
        ...
        # Check data set size
        assert dataset.X_train.shape == torch.Size([283, 72, 80])
        assert dataset.X_val.shape == torch.Size([80, 72, 80])
        assert dataset.X_test.shape == torch.Size([40, 72, 80])
    ...
    def test_time_mask_delta(self):
        ...
        # Check data set size
        assert dataset.X_train.shape == torch.Size([283, 72, 121])
        assert dataset.X_val.shape == torch.Size([80, 72, 121])
        assert dataset.X_test.shape == torch.Size([40, 72, 121])
        # Check time channel
        for i in range(72):
            assert torch.equal(
                dataset.X_train[:, i, 0],
    ...
    def test_downscale(self):
        ...
        # Check data set size
        assert dataset.X_train.shape == torch.Size([565, 72, 41])
        assert dataset.y_train.shape == torch.Size([565, 1])
        assert dataset.length_train.shape == torch.Size([565])
        assert dataset.X_val.shape == torch.Size([161, 72, 41])
        assert dataset.y_val.shape == torch.Size([161, 1])
        assert dataset.length_val.shape == torch.Size([161])
        assert dataset.X_test.shape == torch.Size([80, 72, 41])
        assert dataset.y_test.shape == torch.Size([80, 1])
